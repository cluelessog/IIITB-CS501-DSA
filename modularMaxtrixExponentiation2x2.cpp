/*
 * modularMaxtrixExponentiation2x2.cpp
 *
...
long long int **powZeroToTen(long long int **A,int n,int mod){
	...
	while(n>0){
		if(n % 2 !=0){
			req = matrixMultiply(A,req,2,2,mod);
		}
			A = matrixMultiply(A,A,2,2,mod);
			n /= 2;
	}
	return req;
}
long long int **pow(long long int **A,int N[],int k,int mod){
	...
	while(i<k){
		if(N[k-1-i]!=0){
			y = matrixMultiply(y,powZeroToTen(A,N[k-1-i],mod),2,2,mod); //to find y*A^Ni
		}
